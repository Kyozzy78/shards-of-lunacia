...
export class App {
  ...
  private setMode(mode: ActionMode): void {
    ...
    if (mode === 'ability' && selected.ability.kind === 'shield') { const result = useSelfAbility(this.state, selected.id); if (!result.error) { this.battleRenderer?.shield(selected.id); this.audio.cue('guard'); this.state = result.state; this.status = `${selected.ability.name} activated.`; this.renderBattle(); } return; }
  ...
  private runCommand(command: Parameters<typeof applyCommand>[1]): void { if (this.moving || this.loading || this.enemyBusy) return; const attacker = command.type === 'attack' ? getUnit(this.state, command.unitId) : undefined; const target = command.type === 'attack' ? getUnit(this.state, command.targetId) : undefined; const result = applyCommand(this.state, command); if (result.error) this.status = result.error; else { if (command.type === 'attack') { this.battleRenderer?.attack(command.unitId, command.targetId); this.audio.cue('attack', attacker?.axieClass); if (target?.guarding) { this.battleRenderer?.shield(target.id, true); this.audio.cue('block'); } } else if (command.type === 'guard') { this.battleRenderer?.shield(command.unitId); this.audio.cue('guard'); } this.state = result.state; this.status = this.state.events[0]?.text ?? 'Action complete.'; this.tutorialStep = Math.max(this.tutorialStep, 3); } this.renderBattle(); }
